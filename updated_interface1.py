...
def new_project_page():
    ...
    business_plan=create_business_plan(project_title, scorecard_type, client_category, product_type)

    # Define Project Plan
    project_plan = define_project_plan(project_title, scorecard_type, client_category, product_type)
    # Automatically save data when all fields are filled
    if project_title and scorecard_type and client_category and product_type and (business_plan[0] or business_plan[1] or project_plan[0] or project_plan[1]):
        project_id = insert_project_data(project_title, scorecard_type, client_category, product_type, instructions, business_plan[0],  project_plan[0])
        st.success(f"✅ Le projet a été enregistré avec succès. ID du projet : {project_id}")
        
        # Store the project_id in session state for later use
        st.session_state.project_id = project_id
    else:
        st.error("❌ Veuillez remplir tous les champs nécessaires, y compris au moins un plan (Business ou Project).")
...
def create_business_plan(project_title, scorecard_type, client_category, product_type):
    with st.expander("Create Business Plan"):
        ...
        if st.button("Our Assistant Recommendation", key="business_plan_response"):
        ...
        return objectives, internal_or_external
...
def define_project_plan(project_title, scorecard_type, client_category, product_type):
    with st.expander("Define Project Plan"):
        ...
        # Button to get assistant's response
        if st.button("Our Assistant Recommendation", key="project_plan_response"):
            with st.spinner("Demande en cours..."):
