...
def get_db_connection():
    conn = mysql.connector.connect(
        ...
        database="parametres_du_projet"  # Ensure the database is selected here
    )
    
    # If you are still having issues, you can explicitly call the use command to ensure the database is selected
    cursor = conn.cursor()
    cursor.execute("USE parametres_du_projet")  # Explicitly select the database
    cursor.close()
...
# Function to fetch project data from the database based on project ID
def get_project_data(project_id):
    ...
    project_data = cursor.fetchone()  # Get the first (and only) result
...
def define_project_parameters():
    ...
    # If project_id exists, fetch the project data from the database
    if project_id:
        project_data = get_project_data(project_id)

        if project_data:
            # Place the project overview in an expander for better aesthetics
            with st.expander("📑 **Project Overview**", expanded=True):
                st.write(f"**Project Title:** {project_data['project_title']}")
                st.write(f"**Scorecard Type:** {project_data['scorecard_type']}")
                st.write(f"**Client Category:** {project_data['client_category']}")
                st.write(f"**Product Type:** {project_data['product_type']}")
                st.write(f"**Instructions:**")
                st.markdown(f"> {project_data['instructions']}")
                st.write(f"**Business Plan:**")
                st.markdown(f"> {project_data['business_plan']}")
                st.write(f"**Project Plan:**")
                st.markdown(f"> {project_data['project_plan']}")

            st.markdown("---")

            # Define parameter details and emojis for visual appeal
            parameter_info = {
                "Exclusions": "Certain types of accounts need to be excluded from the development sample.",
                "Performance and Sample Windows": "Scorecards are developed based on the assumption that future performance will reflect past performance.",
                "Definition of 'Bad'": "This involves setting a clear and interpretable definition of what constitutes a 'bad' account.",
                "Definition of 'Good' and 'Indeterminate'": "Defining what constitutes a 'good' account is essential.",
                "Data Availability and Quality": "Reliable and clean data is crucial for scorecard development.",
                "Segmentation": "This involves dividing the data into meaningful segments to improve the predictive power of the scorecard.",
                "Methodology": "The methodology for developing the scorecard should be clearly defined."
            }
            ...
            # Loop through parameters to display expandable input fields for the user
            for param, emo in zip(parameter_info.keys(), emojis):
                with st.container():
                    with st.expander(f"**{emo} {param}**", expanded=True):
                        st.markdown(parameter_info[param])
                    parameter_values[param] = st.text_input(f"Enter {param} for further insights:", key=param)

            st.markdown("---")

            # Checking if all fields are filled before enabling recommendations
            all_filled = all(value for value in parameter_values.values())

            # Generate recommendations only when all fields are entered
            if all_filled:
                # Combine the data (from database + user input) to send to the assistant
                combined_data = {
                    "project_title": project_data['project_title'],
                    "scorecard_type": project_data['scorecard_type'],
                    "client_category": project_data['client_category'],
                    "product_type": project_data['product_type'],
                    "instructions": project_data['instructions'],
                    "business_plan": project_data['business_plan'],
                    "project_plan": project_data['project_plan'],
                    **parameter_values  # Add the user-entered parameters
                }

                with st.spinner("Assistant is analyzing the project parameters..."):
                    recommendation = get_assistant_response(combined_data, "project parameters")
                    st.subheader("💡 Assistant Recommendations")
                    st.write(recommendation)
            else:
                st.warning("⚠️ Please fill in all the fields to receive recommendations from the assistant.")

        else:
            st.error("❌ Project not found.")
    else:
        st.error("❌ Project ID not found. Please create a project first.")
